...
namespace Andrew
{
	...
	public:
		...
		Package lobbiesGet() {
			Package request;
			request.type = PckgType::LobbiesGetReq;
		...
		Package sendFileChunk(u_long id, u_long lobbyId, u_long chunkId, u_long size, u_long offset, const wchar_t* buffer)
		{
			Package request;
			request.type = PckgType::SendFileChunkReq;
		...
		Package getFileChunk(u_long lobbyId, u_long chunkId)
		{
			Package request;
			request.type = PckgType::GetFileChunkReq;
